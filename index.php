  ...
  switch ($act) {
    ...
    case "them_vao_gio_hang":
      if (isset($_POST['btn_insert_cart'])) {
        ...
        //check
        $kh=$_SESSION['tai_khoan'];
        $c=fetch_array($conn,"SELECT * FROM gio_hang WHERE user='{$kh['ma_kh']}' and product = '$ma_hang_hoa' LIMIT 1");
        if($c){
          @mysqli_query($conn,"UPDATE gio_hang set number=number+$so_luong WHERE id='{$c['id']}'");

        }
        else{
          @mysqli_query($conn,"INSERT into gio_hang (user,product,number)values('{$kh['ma_kh']}','$ma_hang_hoa',$so_luong)");
    ...
    case "xoa_gio_hang":
      if (isset($_GET['id'])) {
        ...
        @mysqli_query($conn,"DELETE  FROM gio_hang WHERE id='{$_GET['id']}' LIMIT 1");
      } else {
        $_SESSION['gio_hang'] = [];
      }
      header("Location: index.php?act=gio_hang");
